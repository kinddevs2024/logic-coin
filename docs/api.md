...
Возвращает независимый прогресс игр пользователя: рекорды, предыдущий результат,
текущий и открытый уровни, пройденные уровни, статистику, игровую валюту
(`coins`, `lifetimeCoins`, `spentCoins`, `transferredCoins`) и выбранные косметические
облики. Поддерживаемые локальные идентификаторы включают `longcat`, `gobble`,
`loops` и `brain-tricks`; схема также хранит лучшие ходы/повороты, время решения и
число использованных подсказок по уровню.

### `PUT /games/progress`

Сохраняет полную версионированную карту игрового прогресса. Клиент сначала
объединяет локальное и облачное состояние по `updatedAt`, поэтому прогресс гостя
не теряется после входа в аккаунт.

Клиент начисляет `coin` за завершённые партии и уровни. Скины покупаются из
баланса конкретной игры.

### `POST /games/convert`

Атомарно списывает игровую валюту и зачисляет её в основную копилку по курсу
`10 coin = 1 LC`. Тело: `gameId`, `coins` (кратно 10) и `idempotencyKey`.
Ответ содержит обновлённые `games`, `wallet` и `convertedUnits`.
